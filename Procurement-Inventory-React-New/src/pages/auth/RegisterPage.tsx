...
const RegisterPage = () => {
  ...
  const [form, setForm]         = useState<RegisterFormValues>({ email: '', password: '', confirmPassword: '' });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setLoading(true);
    try {
      const data = await register(form.email, form.password, form.confirmPassword);
    ...
    <div className="min-h-screen flex bg-white font-sans antialiased">
      ...
      <div className="w-full lg:w-[48%] xl:w-[40%] flex flex-col justify-between px-8 sm:px-16 lg:px-24 py-12 bg-white">
        ...
        <div className="my-auto py-8 w-full max-w-md mx-auto">
          <div className="mb-8">
            <h1 className="text-zinc-900 text-3xl font-bold tracking-tight">Create your account</h1>
            <p className="text-zinc-500 text-sm mt-2">Welcome! Please enter your details below.</p>
          </div>
          ...
          {success && (
            <div className="mb-6 p-4 rounded-lg bg-zinc-50 border border-zinc-200 text-sm text-emerald-600 flex items-center gap-2">
              <svg className="w-5 h-5 flex-shrink-0 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>
              <span>{success}</span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm placeholder-gray-400 focus:outline-none focus:border-zinc-900 focus:ring-1 focus:ring-zinc-900 transition-colors ${
            ...
            <div>
              <label htmlFor="confirmPassword" className="text-sm font-medium text-zinc-700 mb-1.5 block">
                Confirm Password
              </label>
              <input
                id="confirmPassword"
              ...
              />
              {errors.confirmPassword && <span className="mt-1.5 block text-xs text-red-500">{errors.confirmPassword}</span>}
            </div>
        ...
        <div className="text-center text-sm text-zinc-500">
          ...
          <Link to="/login" className="font-semibold text-zinc-900 hover:underline transition duration-150 ease-in-out">
            Sign in
          </Link>
        </div>
      </div>
